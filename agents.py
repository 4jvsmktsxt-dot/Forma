...
def render_ai_agent_chat(property_id, user_role="Ostaja"):
    ...
    if chat_key not in st.session_state:
        st.session_state[chat_key] = [
            {
                "role": "assistant", 
                "content": f"Hei! Olen Forman {agent_type.split()[1].lower()}agentti. Näen ladatut kuvat keittiöstä ja ruokailuryhmästä. Miten voin auttaa sinua remontissa tai huonekalujen sijoittelussa?"
            }
        ]
    ...
    if prompt := st.chat_input("Kysy esim. 'Mahtuuko tuo pöytä vierashuoneeseen?' tai 'Paljonko keittiön kaapit maksaa?'..."):
        ...
        with st.chat_message("assistant"):
            ...
            if "Sisustaja" in agent_type:
                if "vierashuone" in lower_prompt or "pöytä" in lower_prompt or "mahtuu" in lower_prompt:
                    response = "Kuvassa näkyvä puinen ruokailupöytä (arviolta ~120 cm pitkä) mahtuu oikein hyvin uuteen vierashuoneeseen, kun sijoitat sen pitkää seinää vasten. Se toimii siellä tarvittaessa myös erinomaisena työpöytänä!"
                else:
                ...
                if "keittiö" in lower_prompt or "kaappi" in lower_prompt or "vaihtaa" in lower_prompt:
                    response = "Keittiön kaappien ja ovien vaihto on loistava tapa uudistaa ilme! Tyypillisen tämän kokoisen keittiön kaapistojen uusiminen (materiaalit + asennustyö avaimet käteen -pakettina) maksaa arviolta 4 500 – 7 000 € riippuen valituista mekanismeista ja tasoista. Haluatko laskea tarkan arvion?"
                else:
                ...
                response = "Kauppavalmentajana varmistan, että remontit ja aikataulut saadaan sovitettua yhteen kaupanteon kanssa. Tehdäänkö tästä kohteesta tarjous?"
